...
async def test_create_ticket_success(async_client: AsyncClient):
    ...
    response = await async_client.post(
        '/tickets/create', json={'price': expected_price}
    )
...
async def test_buy_ticket_success(
    async_session: AsyncSession, async_client: AsyncClient
):
    ...
    response = await async_client.post(
        '/tickets/buy',
        json={'ticket_id': new_ticket.id, 'user': expected_user},
    )
...
async def test_buy_ticket_when_ticket_not_found(
    ...
    response = await async_client.post(
        '/tickets/buy',
        json={'ticket_id': new_ticket.id + 1, 'user': 'other user'},
    )
